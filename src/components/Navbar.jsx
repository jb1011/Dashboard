...
const NavButton = ({ title, customFunc, icon, color, dotColor }) => (
  <TooltipComponent content={title} position="BottomCenter">
    <button type='button' onClick={customFunc} style={{ color }} className="relative text-xl rounded-full p-3 hover:bg-light-gray">
      <span style={{ background: dotColor }} className='absolute inline-flex rounded-full h-2 w-2 right top-2' />
        {icon}
    </button>
  </TooltipComponent>
)
  ...
  return (
    <div className='flex justify-between p-1 md:mx-6 relative'>
      ...
      <div className='flex'>
        ...
        <NavButton title="Cart" dotColor='red' customFunc={() => handleClick('notification')} color={currentColor} icon={<RiNotification3Line />} />
